...
const dragTask = async (req, res) => {
    ...
    if (proj) {
        ...
        const task = proj.tasks[proj.tasks.findIndex((tsk) => tsk.id === req.body.id)];
        let changedTask = false;
        if (task && task.progress !== req.body.newProgress) {
            changedTask = true;
        }
        if (changedTask) {
            task.progress = req.body.newProgress;
        }
        proj.lists[oldListIdx].taskIds = proj.lists[oldListIdx].taskIds.filter((taskId) => taskId !== req.body.id);
