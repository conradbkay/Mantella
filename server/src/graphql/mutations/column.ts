...
const createColumn: MutationResolvers['createColumn'] = async (
    ...
    const [user, project] = await Promise.all([UserModel.findOne({ id: context.userId.id }), ProjectModel.findOne({id: args.projId})])
  
    if(user && project) {
        project.columns.push({inProgress: false, id: creatingId, name: args.name, collapsedUsers: []})
        const newProj = await project.save()
        return {project: newProj, column: newProj.columns.find((col) => col.id === creatingId)}
    }
    throw new AuthenticationError('user id not provided')

}

const toggleCollapsed: MutationResolvers['toggleCollapsed'] = async (parent, args, context) => {
    const [user, project] = await Promise.all([UserModel.findOne({ id: context.userId.id }), await ProjectModel.findOne({id: args.projId})])
...
const deleteColumn: MutationResolvers['deleteColumn'] = async (parent, args, context) => {
